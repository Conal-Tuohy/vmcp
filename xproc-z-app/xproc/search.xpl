<p:library version="1.0" 
	...
	<p:declare-step name="search" type="chymistry:search">
		...
		<cx:message>
			<p:with-option name="message" select="concat('search-fields: ', $search-fields)"/>
		</cx:message>
		<p:choose>
			<p:when test="/c:request/@method='GET'">
				...
				<p:http-request/>
				<z:dump href="/tmp/solr-response"/>
				<p:xslt name="convert-json-to-xml">
					<p:input port="parameters"><p:empty/></p:input>
					<p:input port="stylesheet"><p:document href="../xslt/convert-between-xml-and-json.xsl"/></p:input>
				</p:xslt>
				<p:wrap-sequence name="request-and-response" wrapper="request-and-response">
					<p:input port="source">
						<p:pipe step="field-values" port="result"/>
